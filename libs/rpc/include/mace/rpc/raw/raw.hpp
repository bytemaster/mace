#ifndef _MACE_RPC_RAW_HPP_
#define _MACE_RPC_RAW_HPP_
#include <mace/reflect/reflect.hpp>

#include <mace/rpc/varint.hpp>
#include <mace/rpc/required.hpp>
#include <mace/rpc/errors.hpp>
...
namespace mace { namespace rpc { namespace raw {
    ...
    namespace detail {
      ...
      struct pack_sequence {
         ...
         template<typename T>
         void operator() ( const T& v )const { mace::rpc::raw::pack(s,v); }
         Stream&    s;
      };
      ...
      struct unpack_sequence {
         ...
         template<typename T>
         void operator() ( T& v )const { mace::rpc::raw::unpack(s,v); }
         Stream&  s;
      };
      ...
      struct if_reflected<boost::true_type> {
        template<typename Stream, typename T>
        static inline void pack( Stream& s, const T& v ) { 
          mace::reflect::reflector<T>::visit( pack_object_visitor<Stream,T>( v, s ) );
        }
        template<typename Stream, typename T>
        static inline void unpack( Stream& s, T& v ) { 
          mace::reflect::reflector<T>::visit( unpack_object_visitor<Stream,T>( v, s ) );
        }
      };
      ...
      struct if_fusion_seq<boost::mpl::false_> {
        template<typename Stream, typename T> 
        inline static void pack( Stream& s, const T& v ) {
          if_reflected<typename mace::reflect::reflector<T>::is_defined>::pack(s,v);
        }
        template<typename Stream, typename T> 
        inline static void unpack( Stream& s, T& v ) {
          if_reflected<typename mace::reflect::reflector<T>::is_defined>::unpack(s,v);
        }
      };
    } // namesapce detail
    ...
    inline void pack( Stream& s, const std::pair<Key,Value>& val ) {
      mace::rpc::raw::pack( s, val.first );
      mace::rpc::raw::pack( s, val.second );
    }
    // support for pair!
    template<typename Stream, typename Key, typename Value>
    void unpack( Stream& s, std::pair<Key,Value>& val ) {
      mace::rpc::raw::unpack( s, val.first );
      mace::rpc::raw::unpack( s, val.second );
    }
    ...
    inline void unpack( Stream& s, std::map<Key,Value>& value ) {
      ...
      for( uint32_t i = 0; i < size.value; ++i ) {
        Key k; 
        mace::rpc::raw::unpack(s,k);
        mace::rpc::raw::unpack(s,value[k]);
      }
    }
...
} } } // namespace mace::rpc::raw

#endif // MACE_RPC_RAW_HPP
