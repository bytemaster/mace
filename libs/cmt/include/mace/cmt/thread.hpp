...
namespace cmt {
   ...
   class thread : public abstract_thread {
      public:
        ...
        auto schedule( const Functor& f, const system_clock::time_point& when, 
                            priority prio = priority(), const char* n= "" ) -> future<decltype(f())> {
               typedef decltype(f()) Result;
               typename promise<Result>::ptr p(new promise<Result>());
               task::ptr tsk( new rtask<Result>(f,p,when,std::max(current_priority(),prio),n) );
               async_task(tsk);
               return p;
        }
         ...
         *  @param n a debug name to be associated with this task. 
         */
        void post( const boost::function<void()>& f, const system_clock::time_point& when, 
         ...
         */
        void post( const boost::function<void()>& t, priority p = priority() );
